...
export default function Billing({ search = '' }) {
  ...
  const visibleRows = rows.filter((invoice) =>
    [invoice.id, invoice.client, invoice.paid ? 'paid' : 'due']
      .join(' ')
      .toLowerCase()
      .includes(query)
  )
  ...
  return (
    <>
      ...
      <section className="panel billing-summary">
        <label>Discount <input type="number" min="0" value={discount} onChange={(event) => setDiscount(event.target.value)} placeholder="500" /></label>
