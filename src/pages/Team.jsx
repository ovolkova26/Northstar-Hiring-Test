...
export default function Team({ search = '' }) {
  ...
  const visible = members.filter((member) => {
    const matchesActivity = showInactive ? true : member.active
    const matchesSearch = [member.name, member.role, member.location]
      .join(' ')
      .toLowerCase()
      .includes(query)
        ...
        {visible.map((member) => (
          <article className="person-card" key={member.id}>
            ...
            <div>
              ...
              <div className="meter"><span style={{ width: `${Math.min(member.capacity, 100)}%` }} /></div>
